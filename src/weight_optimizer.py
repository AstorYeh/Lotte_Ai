...
class WeightOptimizer:
    ...
    def should_adjust(self, group_id, accuracy, recent_history):
        ...
        if accuracy < 0.2:
            ...
            # 🔥 強化修補 1: 降低資料需求門檻,更快開始調整
            if len(recent_accuracies) < 2:  # 原本: self.observation_window
                # 資料不足但仍嘗試探索性調整
                return {
                    'should_adjust': True,  # 原本: False
                    'reason': f'資料不足但嘗試探索性調整 ({len(recent_accuracies)}期)',
                    'is_exception': False  # 原本: True
                }
            
            avg_recent = np.mean(recent_accuracies)
            
            # 🔥 強化修補 2: 移除「暫不調整」邏輯,改為分級調整
            if avg_recent < 0.15:
                # 極度低迷,大幅調整
                return {
                    'should_adjust': True,
                    'reason': f'連續極度低迷 (平均 {avg_recent:.0%}),大幅調整',
                    'is_exception': False
                }
            elif avg_recent < 0.25:
                # 持續低迷,中幅調整 (原本這裡會返回 False!)
                return {
                    'should_adjust': True,  # 🔥 關鍵修改: False → True
                    'reason': f'持續低迷 (平均 {avg_recent:.0%}),中幅調整',
                    'is_exception': False
                }
            else:
                # 可能是例外,小幅探索性調整
                return {
                    'should_adjust': True,  # 🔥 關鍵修改: False → True
                    'reason': f'小幅探索性調整 (近期平均 {avg_recent:.0%})',
                    'is_exception': False
                }
    ...
    def backtest_adjustment(self, proposed_weights, historical_data, group_id):
        ...
        try:
            ...
            # 🔥 進階修補 1: 移除拒絕調整邏輯,始終允許但給予不同建議
            if avg_accuracy > 0.3:
                return {
                    'is_valid': True,
                    'message': f'近期表現良好 ({avg_accuracy:.0%}),建議中幅調整',
                    'improvement': 0.05
                }
            elif avg_accuracy > 0.15:
                # 表現一般,小幅調整
                return {
                    'is_valid': True,  # 🔥 關鍵: 改為 True
                    'message': f'近期表現一般 ({avg_accuracy:.0%}),建議小幅調整',
                    'improvement': 0.02
                }
            else:
                # 表現不佳,探索性調整 (原本會拒絕!)
                return {
                    'is_valid': True,  # 🔥 關鍵: 改為 True
                    'message': f'近期表現不佳 ({avg_accuracy:.0%}),探索性調整',
                    'improvement': 0.01
                }
