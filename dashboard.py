...
# [CRITICAL] 確保當前目錄在 sys.path 中，解決 src import 問題
current_dir = os.path.dirname(os.path.abspath(__file__))
if current_dir not in sys.path:
    sys.path.append(current_dir)

# 頁面配置 (必須是第一個 Streamlit 指令)
st.set_page_config(
...
def main():
    # [DEFENSIVE IMPORT] 將所有依賴移入 main，避免啟動時 Crash
    try:
        import pandas as pd
        ...
        import traceback
        st.error("❌ 系統啟動失敗 (Startup Failed)")
        st.error(f"錯誤訊息: {str(e)}")
        with st.expander("查看詳細錯誤資訊 (Debug Info)"):
