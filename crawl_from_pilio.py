# -*- coding: utf-8 -*-
"""
Pilio 網站爬蟲
從 https://www.pilio.idv.tw/ 爬取所有遊戲的歷史資料
"""
import requests
from bs4 import BeautifulSoup
import pandas as pd
from datetime import datetime
from pathlib import Path
import time
import re


def fetch_539_from_pilio(max_pages=100):
    """從 Pilio 爬取 539 歷史資料"""
    print("\n" + "=" * 80)
    print("[1/5] 從 Pilio 爬取 539 歷史資料")
    print("=" * 80)
    
    results = []
    base_url = "https://www.pilio.idv.tw/lto539/list.asp"
    
    for page in range(1, max_pages + 1):
        try:
            url = f"{base_url}?indexpage={page}&orderby=new"
            print(f"\n  頁面 {page}/{max_pages}: {url}")
            
            response = requests.get(url, timeout=10)
            response.encoding = 'big5'  # Pilio 使用 big5 編碼
            
            soup = BeautifulSoup(response.text, 'html.parser')
            
            # 找到所有開獎記錄的表格行
            rows = soup.find_all('tr')
            
            page_count = 0
            for row in rows:
                cols = row.find_all('td')
                if len(cols) < 3:
                    continue
                
                # 提取日期
                date_text = cols[1].get_text(strip=True) if len(cols) > 1 else ""
                
                # 檢查是否為有效日期格式
                date_match = re.search(r'(\d{4})/(\d{1,2})/(\d{1,2})', date_text)
                if not date_match:
                    continue
                
                date_str = f"{date_match.group(1)}-{date_match.group(2).zfill(2)}-{date_match.group(3).zfill(2)}"
                
                # 提取號碼
                numbers = []
                for col in cols[2:]:  # 從第3欄開始是號碼
                    text = col.get_text(strip=True)
                    if text.isdigit() and 1 <= int(text) <= 39:
                        numbers.append(int(text))
                    if len(numbers) >= 5:
                        break
                
                if len(numbers) == 5:
                    results.append({
                        'date': date_str,
                        'numbers': ','.join([str(n) for n in sorted(numbers)])
                    })
                    page_count += 1
            
            print(f"    找到 {page_count} 筆資料")
            
            if page_count == 0:
                print(f"    頁面 {page} 無資料,停止爬取")
                break
            
            time.sleep(0.5)
            
        except Exception as e:
            print(f"    [ERROR] 頁面 {page} 失敗: {e}")
            break
    
    # 儲存
    if results:
        df = pd.DataFrame(results)
        df = df.drop_duplicates(subset=['date'])
        df = df.sort_values('date')
        df.to_csv("data/539_history.csv", index=False)
        print(f"\n[OK] 539 完成: {len(df)} 筆資料")
    else:
        print(f"\n[WARNING] 539 無資料")
    
    return len(results)


def fetch_lotto_from_pilio(max_pages=50):
    """從 Pilio 爬取大樂透歷史資料"""
    print("\n" + "=" * 80)
    print("[2/5] 從 Pilio 爬取大樂透歷史資料")
    print("=" * 80)
    
    results = []
    base_url = "https://www.pilio.idv.tw/ltobest/list.asp"
    
    for page in range(1, max_pages + 1):
        try:
            url = f"{base_url}?indexpage={page}&orderby=new"
            print(f"\n  頁面 {page}/{max_pages}: {url}")
            
            response = requests.get(url, timeout=10)
            response.encoding = 'big5'
            
            soup = BeautifulSoup(response.text, 'html.parser')
            rows = soup.find_all('tr')
            
            page_count = 0
            for row in rows:
                cols = row.find_all('td')
                if len(cols) < 3:
                    continue
                
                date_text = cols[1].get_text(strip=True) if len(cols) > 1 else ""
                date_match = re.search(r'(\d{4})/(\d{1,2})/(\d{1,2})', date_text)
                if not date_match:
                    continue
                
                date_str = f"{date_match.group(1)}-{date_match.group(2).zfill(2)}-{date_match.group(3).zfill(2)}"
                
                numbers = []
                for col in cols[2:]:
                    text = col.get_text(strip=True)
                    if text.isdigit() and 1 <= int(text) <= 49:
                        numbers.append(int(text))
                    if len(numbers) >= 6:
                        break
                
                if len(numbers) == 6:
                    results.append({
                        'date': date_str,
                        'numbers': ','.join([str(n) for n in sorted(numbers)])
                    })
                    page_count += 1
            
            print(f"    找到 {page_count} 筆資料")
            
            if page_count == 0:
                break
            
            time.sleep(0.5)
            
        except Exception as e:
            print(f"    [ERROR] 頁面 {page} 失敗: {e}")
            break
    
    if results:
        Path("data/lotto").mkdir(parents=True, exist_ok=True)
        df = pd.DataFrame(results)
        df = df.drop_duplicates(subset=['date'])
        df = df.sort_values('date')
        df.to_csv("data/lotto/lotto_history.csv", index=False)
        print(f"\n[OK] 大樂透完成: {len(df)} 筆資料")
    else:
        print(f"\n[WARNING] 大樂透無資料")
    
    return len(results)


def fetch_power_from_pilio(max_pages=100):
    """從 Pilio 爬取威力彩歷史資料"""
    print("\n" + "=" * 80)
    print("[3/5] 從 Pilio 爬取威力彩歷史資料")
    print("=" * 80)
    
    results = []
    base_url = "https://www.pilio.idv.tw/lto/list.asp"
    
    for page in range(1, max_pages + 1):
        try:
            url = f"{base_url}?indexpage={page}&orderby=new"
            print(f"\n  頁面 {page}/{max_pages}: {url}")
            
            response = requests.get(url, timeout=10)
            response.encoding = 'big5'
            
            soup = BeautifulSoup(response.text, 'html.parser')
            rows = soup.find_all('tr')
            
            page_count = 0
            for row in rows:
                cols = row.find_all('td')
                if len(cols) < 3:
                    continue
                
                date_text = cols[1].get_text(strip=True) if len(cols) > 1 else ""
                date_match = re.search(r'(\d{4})/(\d{1,2})/(\d{1,2})', date_text)
                if not date_match:
                    continue
                
                date_str = f"{date_match.group(1)}-{date_match.group(2).zfill(2)}-{date_match.group(3).zfill(2)}"
                
                zone1 = []
                zone2 = None
                for col in cols[2:]:
                    text = col.get_text(strip=True)
                    if text.isdigit():
                        num = int(text)
                        if len(zone1) < 6 and 1 <= num <= 38:
                            zone1.append(num)
                        elif len(zone1) == 6 and 1 <= num <= 8:
                            zone2 = num
                            break
                
                if len(zone1) == 6 and zone2 is not None:
                    results.append({
                        'date': date_str,
                        'zone1': str(sorted(zone1)),
                        'zone2': zone2
                    })
                    page_count += 1
            
            print(f"    找到 {page_count} 筆資料")
            
            if page_count == 0:
                break
            
            time.sleep(0.5)
            
        except Exception as e:
            print(f"    [ERROR] 頁面 {page} 失敗: {e}")
            break
    
    if results:
        Path("data/power").mkdir(parents=True, exist_ok=True)
        df = pd.DataFrame(results)
        df = df.drop_duplicates(subset=['date'])
        df = df.sort_values('date')
        df.to_csv("data/power/power_history.csv", index=False)
        print(f"\n[OK] 威力彩完成: {len(df)} 筆資料")
    else:
        print(f"\n[WARNING] 威力彩無資料")
    
    return len(results)


def fetch_star3_from_pilio(max_pages=100):
    """從 Pilio 爬取 3星彩歷史資料"""
    print("\n" + "=" * 80)
    print("[4/5] 從 Pilio 爬取 3星彩歷史資料")
    print("=" * 80)
    
    results = []
    base_url = "https://www.pilio.idv.tw/lto/list3.asp"
    
    for page in range(1, max_pages + 1):
        try:
            url = f"{base_url}?indexpage={page}&orderby=new"
            print(f"\n  頁面 {page}/{max_pages}: {url}")
            
            response = requests.get(url, timeout=10)
            response.encoding = 'big5'
            
            soup = BeautifulSoup(response.text, 'html.parser')
            rows = soup.find_all('tr')
            
            page_count = 0
            for row in rows:
                cols = row.find_all('td')
                if len(cols) < 3:
                    continue
                
                date_text = cols[1].get_text(strip=True) if len(cols) > 1 else ""
                date_match = re.search(r'(\d{4})/(\d{1,2})/(\d{1,2})', date_text)
                if not date_match:
                    continue
                
                date_str = f"{date_match.group(1)}-{date_match.group(2).zfill(2)}-{date_match.group(3).zfill(2)}"
                
                # 3星彩是3位數字
                number_text = cols[2].get_text(strip=True) if len(cols) > 2 else ""
                if len(number_text) == 3 and number_text.isdigit():
                    results.append({
                        'date': date_str,
                        'number': number_text
                    })
                    page_count += 1
            
            print(f"    找到 {page_count} 筆資料")
            
            if page_count == 0:
                break
            
            time.sleep(0.5)
            
        except Exception as e:
            print(f"    [ERROR] 頁面 {page} 失敗: {e}")
            break
    
    if results:
        Path("data/star3").mkdir(parents=True, exist_ok=True)
        df = pd.DataFrame(results)
        df = df.drop_duplicates(subset=['date'])
        df = df.sort_values('date')
        df.to_csv("data/star3/star3_history.csv", index=False)
        print(f"\n[OK] 3星彩完成: {len(df)} 筆資料")
    else:
        print(f"\n[WARNING] 3星彩無資料")
    
    return len(results)


def fetch_star4_from_pilio(max_pages=100):
    """從 Pilio 爬取 4星彩歷史資料"""
    print("\n" + "=" * 80)
    print("[5/5] 從 Pilio 爬取 4星彩歷史資料")
    print("=" * 80)
    
    results = []
    base_url = "https://www.pilio.idv.tw/lto/list4.asp"
    
    for page in range(1, max_pages + 1):
        try:
            url = f"{base_url}?indexpage={page}&orderby=new"
            print(f"\n  頁面 {page}/{max_pages}: {url}")
            
            response = requests.get(url, timeout=10)
            response.encoding = 'big5'
            
            soup = BeautifulSoup(response.text, 'html.parser')
            rows = soup.find_all('tr')
            
            page_count = 0
            for row in rows:
                cols = row.find_all('td')
                if len(cols) < 3:
                    continue
                
                date_text = cols[1].get_text(strip=True) if len(cols) > 1 else ""
                date_match = re.search(r'(\d{4})/(\d{1,2})/(\d{1,2})', date_text)
                if not date_match:
                    continue
                
                date_str = f"{date_match.group(1)}-{date_match.group(2).zfill(2)}-{date_match.group(3).zfill(2)}"
                
                # 4星彩是4位數字
                number_text = cols[2].get_text(strip=True) if len(cols) > 2 else ""
                if len(number_text) == 4 and number_text.isdigit():
                    results.append({
                        'date': date_str,
                        'number': number_text
                    })
                    page_count += 1
            
            print(f"    找到 {page_count} 筆資料")
            
            if page_count == 0:
                break
            
            time.sleep(0.5)
            
        except Exception as e:
            print(f"    [ERROR] 頁面 {page} 失敗: {e}")
            break
    
    if results:
        Path("data/star4").mkdir(parents=True, exist_ok=True)
        df = pd.DataFrame(results)
        df = df.drop_duplicates(subset=['date'])
        df = df.sort_values('date')
        df.to_csv("data/star4/star4_history.csv", index=False)
        print(f"\n[OK] 4星彩完成: {len(df)} 筆資料")
    else:
        print(f"\n[WARNING] 4星彩無資料")
    
    return len(results)


if __name__ == "__main__":
    print("=" * 80)
    print("從 Pilio 網站爬取歷史資料")
    print("資料來源: https://www.pilio.idv.tw/")
    print("=" * 80)
    
    start_time = time.time()
    total_records = 0
    
    # 依序爬取各遊戲 (每個遊戲最多100頁,約可獲取2-3年資料)
    total_records += fetch_539_from_pilio(100)
    total_records += fetch_lotto_from_pilio(100)
    total_records += fetch_power_from_pilio(100)
    total_records += fetch_star3_from_pilio(100)
    total_records += fetch_star4_from_pilio(100)
    
    elapsed = time.time() - start_time
    
    print("\n" + "=" * 80)
    print("爬取完成!")
    print("=" * 80)
    print(f"總筆數: {total_records}")
    print(f"耗時: {elapsed/60:.1f} 分鐘")
    print("=" * 80)
